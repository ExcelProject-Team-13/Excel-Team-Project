...
const PieChart3D = () => {
    ...
    async function init() {
        ...
        async function createPieAndLabels() {
            const total = Data.reduce((acc, slice) => acc + Number(slice[Headers[1]]), 0);
            const radius = 5;
            const height = 0.5;
            let startAngle = 0;

            for (const slice of Data) {
                const sliceAngle = (Number(slice[Headers[1]]) / total) * Math.PI * 2;
                ...
                const z = Math.cos(midAngle) * labelRadius;

                // Changed from slice.Value to Number(slice[Headers[1]])
                const percentage = ((Number(slice[Headers[1]]) / total) * 100).toFixed(1) + "%";
                try {
                    const labelMesh = await textLoader(`${slice[Headers[0]]} (${percentage})`, 0.4, x, y, z);
                    PIE.add(labelMesh);
                } catch (err) {
