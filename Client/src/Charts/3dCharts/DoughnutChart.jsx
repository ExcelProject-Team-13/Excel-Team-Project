...
const DoughnutChart = () => {
    ...
    const [chartTitle, setChartTitle] = useState("Doughnut Chart")
    ...
    async function init() {
        ...
        const radius = 4;
        const tube = 2;
        const heightTUbe = tube * 0.01;
        const radialSegments = 16;
        const tubularSegments = 50;
//gives the total values in the dummy data
        const total = dummyData.reduce((sum, d) => sum + d.Value, 0);
        let startAngle = 0;



      for (const slice of dummyData) {
    const sliceAngle = (slice.Value / total) * Math.PI * 2;

    // Segment (doughnut slice)
    const geometry = new THREE.TorusGeometry(radius, tube, radialSegments, tubularSegments, sliceAngle);
    ...
    const y = heightTUbe + 2;
    const z = Math.sin(midAngle) * labelRadius;

    const percentage = ((slice.Value / total) * 100).toFixed(1) + "%";
    try {
        const labelMesh = await textLoader(`${slice.Category} (${percentage})`, 0.4, x, y, z);
        scene.add(labelMesh);
    } catch (err) {
        console.log("Text loading failed", err);
    }

    // Line from middle of arc to label
    const lineStartX = Math.cos(midAngle) * radius;
    const lineStartZ = Math.sin(midAngle) * radius;
    const lineStartY = heightTUbe / 10;

    const points = [
        new THREE.Vector3(lineStartX, lineStartY, lineStartZ),
    ...
    startAngle += sliceAngle;
        ...
        try {
            const titleMesh = await textLoader(chartTitle, 1.2, 0, 0, -9);
            scene.add(titleMesh);
        } catch (err) {
    ...
    // Setup initial dummyData only once
    useEffect(() => {
        init();
    }, [Data, width, height]);
   ...
   const submit = () => {
    ...
    return (
        <div className="w-[100vw] overflow-x-hidden relative pt-7 md:px-10 px-4 pb-10">
            ...
            <h1 className="text-3xl w-full text-center font-bold mb-5">Doughnut CHART</h1>
