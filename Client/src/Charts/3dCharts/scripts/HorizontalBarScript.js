...
const HorizontalBarChart3D = forwardRef(({ importingData }, ref) => {
  ...
  const init = async () => {
    ...
    // +X,+Y plane
    for (let x = 0; x <= X * boxSize; x += boxSize) {
      const points = [new THREE.Vector3(x, 0, Z * boxSize), new THREE.Vector3(x, (Y / Step), Z * boxSize)];
      gridLines.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints(points), new THREE.LineBasicMaterial({ color: gridColor })));
    }
    for (let y = 0; y <= ((Y / Step) * boxSize) + 1; y += boxSize) {
      const points = [new THREE.Vector3(0, y / boxSize, Z * boxSize), new THREE.Vector3(X * boxSize, y / boxSize, Z * boxSize)];
      gridLines.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints(points), new THREE.LineBasicMaterial({ color: gridColor })));
    }
    ...
    for (let i = 0; i < labels.length; i++) {
      const textMesh = await createText(font, String(labels[i]), 0.2, -0.5, i, Z * boxSize);
    ...
    for (let i = 0; i < importingData.series.length; i++) {
      const textMesh = await createText(font, importingData.series[i].name, 0.2, -1, 0, i * boxSize + boxSize / 2);
      textGroup.add(textMesh);
    }

    for (let i = 0; i < importingData.labels.length; i++) {
      const textMesh = await createText(font, importingData.labels[i], 0.2, i * boxSize + boxSize / 2, Y / Step + 0.5, Z * boxSize + 0.5);
      textGroup.add(textMesh);
    }
    ...
    if (importingData.title) {
      titleMesh = await createText(font, importingData.title, 0.6, X * boxSize + 1, 0, -2);
      scene.add(titleMesh);
    }
...
export default memo(HorizontalBarChart3D);