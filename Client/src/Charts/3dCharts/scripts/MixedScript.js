...
const MixedBarLineChart3D = forwardRef(({ importingData }, ref) => {
    const { width, height } = useResponsiveDimensions();
    const [dummyData, setdummyData] = useState({
        labels: ['Mar', 'Apr', 'May'],
        title: 'MixedBarLine Chart',
        series: [
    ...
    async function init() {
        ...
        function createBarsAndLines(data) {
            const lineGroup = new THREE.Group();
            const pointGroup = new THREE.Group(); // Group for spheres

            for (let row = 0; row < data.series.length; row++) {
                const { values, color } = data.series[row];
                const points = [];
                ...
                for (let col = 0; col < values.length; col++) {
                    ...
                    const bar = new THREE.Mesh(
                        new THREE.BoxGeometry(1, value / Step, 1),
                        new THREE.MeshStandardMaterial({ color })
                    );
                    const x = row * boxSize + boxSize / 2;
                    const z = col * boxSize + boxSize / 2;
                    const y = value / (2 * Step);
                    bar.position.set(x, y, z);
                    barGroup.add(bar);

                    // Line point (same center, top of bar)
                    const point = new THREE.Vector3(x, value / Step + 0.5, z);
                    points.push(point);

                    // Sphere at the line point
            ...
            scene.add(barGroup);
            scene.add(lineGroup);
            scene.add(pointGroup); // Add spheres to the scene
        }

        createBarsAndLines(dummyData);
        // === Creating Title ===
        if (!dummyData.title) {
            return;
        } else {
            textLoader(dummyData.title, 0.4, X * boxSize , Y / Step + 1, 0)
        }
...
export default memo(MixedBarLineChart3D, (prevProps, nextProps) => {
